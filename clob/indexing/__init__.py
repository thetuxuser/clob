...
DEFAULT_IGNORES = {
    "__pycache__", "*.pyc", "*.pyo", ".git", ".svn", "node_modules",
    ".venv", "venv", "env", ".env", "dist", "build", "*.egg-info",
    ".ruff_cache", ".mypy_cache", ".pytest_cache", "*.log", "*.db",
    "*.sqlite", ".DS_Store",
}
BINARY_EXTS = {
    ".png",".jpg",".jpeg",".gif",".webp",".ico",".pdf",".zip",
    ".tar",".gz",".whl",".exe",".bin",".pyc",".so",".dylib",".sqlite",
}
...
class WorkspaceIndex:
    ...
    @property
    def total_files(self) -> int: return len(self.files)
    @property
    def total_tokens(self) -> int: return sum(f.estimated_tokens for f in self.files)
    @property
    def total_size_bytes(self) -> int: return sum(f.size_bytes for f in self.files)
